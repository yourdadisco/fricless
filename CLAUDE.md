...
> **绝不允许将问题归咎于 AI 模型。**
>
> 任何功能缺陷、输出错误、行为异常，100% 是代码的问题。没有"模型不行"这个理由。
> 1. AI 不按预期行事 → 提示词写得不够好，或者工具设计有缺陷
> 2. AI 编造信息 → 工具没把真实结果正确传递给用户，或者提示词没约束住
> 3. AI 不用工具 → 工具的描述、触发条件、返回值设计不合理
> 4. 修复方向永远是改进代码，而不是等模型变聪明
